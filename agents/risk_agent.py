...
class RiskManagementAgent(BaseAgent):
    ...
    async def analyze(self, symbol: str, data: pd.DataFrame, market_data: Dict) -> TradingSignal:
        ...
        Portfolio Risk Score: {risk_metrics['portfolio_risk']:.2f}
        Position Risk: {risk_metrics['position_risk']:.2f}
        Volatility: {risk_metrics['volatility']:.2f}
        Sector Concentration: {risk_metrics['sector_risk']:.2f}
        Cash Ratio: {risk_metrics['cash_ratio']:.2f}
        
        Recommended Action: {action}
        Risk Level: {risk_metrics['overall_risk_level']}
        
        Provide risk management recommendations and position sizing guidance.
        """
    ...
    async def _calculate_sector_risk(self, positions: List[Dict], new_symbol: str) -> float:
        ...
        # Risk increases exponentially with sector concentration
    ...
    def _calculate_risk_adjusted_position_size(self, risk_metrics: Dict, portfolio_value: float) -> float:
        ...
        base_size = self._calculate_position_size(0.5, portfolio_value)  # Base 5% position
